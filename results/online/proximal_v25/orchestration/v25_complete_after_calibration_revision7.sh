...
QUEUE_SCRIPT=/home/carla/LZQW/SAFE100/humanoid/artifacts/v25_wait_and_calibrate_revision7.sh
...
WATCHDOG_SCRIPT=/home/carla/LZQW/SAFE100/humanoid/artifacts/v25_supervisor_watchdog_revision7.sh
...
exec >>"$ORCHESTRATION_ROOT/completion_orchestrator_revision7.log" 2>&1
...
queue_active() {
  ...
  pgrep -f '^bash /home/carla/LZQW/SAFE100/humanoid/artifacts/v25_wait_and_calibrate_revision7\.sh$' \
    >/dev/null 2>&1
}
...
require_gpu_clear_now() {
  ...
  if ! guardian_release_authorized || guardian_active || guardian_watchdog_active; then
    fail "Guardian release changed during the final prelaunch check for $CURRENT_PHASE"
  fi
  mapfile -t active_pids < <(foreign_compute_pids)
  mapfile -t active_scheduler_pids < <(foreign_gpu_scheduler_pids)
  memory_used_mib=$(gpu_memory_used_mib)
  (( ${#active_scheduler_pids[@]} == 0 )) || fail \
    "foreign GPU scheduler appeared during final prelaunch check for $CURRENT_PHASE: ${active_scheduler_pids[*]}"
  (( ${#active_pids[@]} == 0 )) || fail \
    "foreign GPU compute appeared during final prelaunch check for $CURRENT_PHASE: ${active_pids[*]}"
  [[ "$memory_used_mib" =~ ^[0-9]+$ ]] || fail \
    "could not read GPU memory during final prelaunch check for $CURRENT_PHASE"
  (( memory_used_mib <= MAX_IDLE_GPU_MEMORY_MIB )) || fail \
    "GPU memory changed during final prelaunch check for $CURRENT_PHASE: used_mib=$memory_used_mib maximum_idle_mib=$MAX_IDLE_GPU_MEMORY_MIB"
  echo "$(timestamp) final prelaunch GPU check passed for $CURRENT_PHASE memory_used_mib=$memory_used_mib"
}
...
wait_for_clean_gpu() {
  ...
  local -a active_scheduler_pids=()
  local memory_used_mib
  while (( clean_samples < CLEAN_GPU_REQUIRED_SAMPLES )); do
    if ! guardian_release_authorized || guardian_active || guardian_watchdog_active; then
      clean_samples=0
      echo "$(timestamp) waiting before $CURRENT_PHASE; Guardian release is invalid, active, or restart-watchdog-managed"
      sleep 60
      continue
    fi
    mapfile -t active_pids < <(foreign_compute_pids)
    ...
    if (( ${#active_scheduler_pids[@]} > 0 )); then
      clean_samples=0
      echo "$(timestamp) waiting before $CURRENT_PHASE; foreign GPU scheduler PIDs=${active_scheduler_pids[*]}"
    elif (( ${#active_pids[@]} > 0 )); then
      clean_samples=0
      echo "$(timestamp) waiting before $CURRENT_PHASE; foreign GPU PIDs=${active_pids[*]}"
    elif [[ ! "$memory_used_mib" =~ ^[0-9]+$ ]] || \
         (( memory_used_mib > MAX_IDLE_GPU_MEMORY_MIB )); then
      clean_samples=0
      echo "$(timestamp) waiting before $CURRENT_PHASE; GPU memory used MiB=${memory_used_mib:-unknown} maximum idle MiB=$MAX_IDLE_GPU_MEMORY_MIB"
    else
      clean_samples=$((clean_samples + 1))
      echo "$(timestamp) GPU clean sample $clean_samples/$CLEAN_GPU_REQUIRED_SAMPLES before $CURRENT_PHASE memory_used_mib=$memory_used_mib"
    fi
    if (( clean_samples < CLEAN_GPU_REQUIRED_SAMPLES )); then
...
copy_orchestration_sources() {
  copy_exact "$QUEUE_SCRIPT" "$RESULT_ROOT/orchestration/v25_wait_and_calibrate_revision7.sh"
  copy_exact "$COMPLETION_SCRIPT" "$RESULT_ROOT/orchestration/v25_complete_after_calibration_revision7.sh"
  copy_exact "$SUPERVISOR_SCRIPT" "$RESULT_ROOT/orchestration/v25_completion_supervisor_revision7.sh"
  copy_exact "$WATCHDOG_SCRIPT" "$RESULT_ROOT/orchestration/v25_supervisor_watchdog_revision7.sh"
...
terminal_receipt = pathlib.Path(sys.argv[6])
if terminal_receipt.is_file():
    files.append(
        {
...
payload = {
    "schema_version": 1,
    "precalibration_revision": 7,
    "frozen_initial_git_commit": sys.argv[7],
...
while queue_active; do
  latest=$(tail -n 1 "$EXTERNAL_ROOT/calibration_queue_revision7.log" 2>/dev/null || true)
