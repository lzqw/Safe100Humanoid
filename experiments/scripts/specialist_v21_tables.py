"""Deterministic machine-readable training tables for specialist v21."""
...
def _accepted_target_eval(record: dict[str, Any]) -> dict[str, Any]:
  ...
  blocks = confirmation.get("blocks") or []
  if record.get("policy_changed_at_round_end"):
    return (blocks[0].get("candidate") if blocks else None) or {}
  return (
    (blocks[0].get("old") if blocks else None)
...
def _current_target_eval(record: dict[str, Any]) -> dict[str, Any]:
  blocks = record.get("candidate_confirmation", {}).get("blocks") or []
  return (
    (blocks[0].get("old") if blocks else None)
    or record.get("candidate_screening", {}).get("old")
    or {}
  ...
  for record in summary["rounds"]:
    ...
    row: dict[str, Any] = {
      ...
      "target_diagnostic_seed": confirmation.get(
        "seed_base", record["candidate_screening"]["seed"]
      ),
      ...
      "confirmation_success_delta": _metric(
        confirmation_deltas, "mean_success_delta"
      ),
      "confirmation_fall_delta": _metric(
        confirmation_deltas, "mean_fall_delta"
      ),
      ...
      rows.append(
        {
          ...
          "confirmation_seed": confirmation["seed_base"],
          "confirmation_success_delta": _metric(
            deltas, "mean_success_delta"
          ),
          "confirmation_fall_delta": _metric(
            deltas, "mean_fall_delta"
          ),
          "target_gate_accepted": record["target_gate_accepted"],
          "d0_rollback": record["d0_rollback"],
