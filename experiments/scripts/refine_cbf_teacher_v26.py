...
def _validate_frozen_protocol(
    ...
    *,
    ...
    if not all(checks.values()):
        raise RuntimeError(f"v26 frozen protocol validation failed: {checks}")
    return {
...
def _algorithm_audit(runner, env_cfg, shift_metadata) -> dict[str, Any]:
    from src.tasks.stairs_cbf.teacher import CbfTeacherPPO
    ...
    checks = {
        ...
        "action_config_class": isinstance(action_cfg, HigherRiserCbfActionCfg),
        ...
        "higher_riser_shift": shift_metadata["shift"]
        == "fixed_uniform_higher_riser",
        "sloped_clearance_barrier": shift_metadata["clearance_barrier"]
        == "sloped_xz",
        "clearance_barrier_slope": shift_metadata["clearance_barrier_slope"],
    ...
    expected = {
        ...
        "runtime_filter": True,
        "higher_riser_shift": True,
        "sloped_clearance_barrier": True,
    ...
    if mismatches:
        raise RuntimeError(f"v26 structural invariant failed: {mismatches}")
    return checks
...
def _save_checkpoint(
    ...
    *,
    ...
    payload["infos"] = {"cbf_teacher_v26": metadata}
        ...
        try:
            ...
            if any(token in message for token in routing_tokens):
                raise ProximalHardRollback(
                    "v26 action/teacher routing audit failed",
...
def main() -> None:
    ...
    if _git_output(repo, "status", "--porcelain"):
        raise RuntimeError("v26 adaptation requires a clean committed worktree")
    if not checkpoint.is_file() or not context_path.is_file():
        raise FileNotFoundError("base checkpoint or v26 context does not exist")
    ...
    if not args.smoke:
        ...
        if mismatches:
            raise ValueError(f"formal v26 execution mismatch: {mismatches}")
        if args.protocol is None:
            raise ValueError("formal v26 execution requires a frozen protocol")
        if (output_dir / "formal_execution_started.json").exists():
            raise RuntimeError("formal v26 adaptation has already been started")
    ...
    # silently introduce a second shift beyond the selected riser height.
    ...
    try:
        ...
        if (
            ...
            raise RuntimeError("v26 warm start changed the observation interface")
