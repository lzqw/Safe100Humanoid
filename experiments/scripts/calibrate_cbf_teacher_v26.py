"""Run ordered paired base-only rescue calibration for v26."""
    ...
    *,
    ...
    if summary is None:
        ...
        if completed.returncode:
            ...
            raise RuntimeError(
                f"v26 calibration arm failed ({height=}, {runtime_filter=}):\n{diagnostic}"
            )
        summary = json.loads(output_json.read_text())
    with output_csv.open(newline="") as handle:
        rows = list(csv.DictReader(handle))
    if len(rows) != EVAL_BATCH_SIZE:
        raise RuntimeError("v26 calibration arm has an incomplete episode table")
    return summary, rows
...
def main() -> None:
    ...
    if subprocess.run(
        ...
        raise RuntimeError("v26 calibration requires a clean committed worktree")
    ...
    if file_sha256(checkpoint) != BASE_CHECKPOINT_SHA256:
        raise RuntimeError("v26 calibration checkpoint differs from frozen pi0")
    ...
    if (
        ...
        raise RuntimeError("v26 pre-calibration protocol is not the frozen input")
    if (
        precalibration.get("revision") != PRECALIBRATION_REVISION
        or precalibration.get("environment") != fixed_environment_parameters()
        or precalibration.get("prospective_freeze", {}).get(
            "before_any_formal_v26_simulator_episode"
        ) is not True
    ):
        raise RuntimeError(
            f"v26 revision-{PRECALIBRATION_REVISION} fixed-environment boundary "
            "is missing"
        )
    ...
    if subprocess.run(
        ...
        raise RuntimeError("v26 frozen implementation is not an ancestor of HEAD")
    ...
    if set(bound_sources) != set(SOURCE_FILES) or any(
        ...
        raise RuntimeError("v26 calibration source differs from pre-calibration freeze")
    ...
    if committed_precalibration != precalibration_path.read_bytes():
        raise RuntimeError("v26 pre-calibration protocol is not committed at HEAD")

    output_dir.mkdir(parents=True, exist_ok=True)
    started = output_dir / "calibration_execution_started.json"
    if started.exists() and not args.resume:
        raise RuntimeError("v26 calibration has already started; use --resume")
    ...
    for candidate_index, height in enumerate(CALIBRATION_HEIGHTS_M):
        off_summaries = []
        on_summaries = []
        ...
        for repeat in range(CALIBRATION_REPEATS):
            ...
            if (
                ...
                raise RuntimeError("v26 off/on arms do not share initial conditions")
            if off_summary["actor_state_sha256"] != on_summary["actor_state_sha256"]:
                raise RuntimeError("v26 off/on arms do not share pi0")
        ...
        if [identity(row) for row in off_rows] != [identity(row) for row in on_rows]:
            raise RuntimeError("v26 paired calibration identities differ")
        actor_hashes = {
            summary["actor_state_sha256"] for summary in (*off_summaries, *on_summaries)
        }
        if len(actor_hashes) != 1:
            raise RuntimeError("v26 calibration changed pi0 between paired batches")
    ...
    if selected is None:
        ...
        raise RuntimeError("no v26 higher-riser candidate entered the rescue corridor")

    parameters = {
        "context_id": CONTEXT_ID,
        "family": CONTEXT_FAMILY,
        "selected_candidate_index": selected["candidate_index"],
        "riser_height_m": selected["riser_height_m"],
    ...
    context = {
        ...
        "calibration": {
            "kind": "base_policy_paired_cbf_rescue_first_qualifier_v26",
            "base_policy_only": True,
            "adapted_policy_evaluations_used": False,
            "precalibration_protocol_sha256": file_sha256(precalibration_path),
            "attempts": attempts,
    ...
    summary = {
        ...
        "selected_candidate_index": selected["candidate_index"],
        "selected_riser_height_m": selected["riser_height_m"],
