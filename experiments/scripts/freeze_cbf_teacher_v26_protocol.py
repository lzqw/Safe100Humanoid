"""Freeze selected v26 shift before the sole eight-round adaptation."""
...
from freeze_cbf_teacher_v26_precalibration import (
...
def _committed_file(repo: Path, path: Path, commit: str) -> dict[str, str]:
    ...
    if content != blob:
        raise RuntimeError(f"formal v26 input is not committed: {relative}")
    return {
        "file": str(relative),
...
def main() -> None:
    ...
    if _git_output(repo, "status", "--porcelain"):
        raise RuntimeError("worktree must be clean before formal v26 freeze")
    if file_sha256(checkpoint) != BASE_CHECKPOINT_SHA256:
        raise RuntimeError("formal v26 checkpoint differs from frozen pi0")
    if (formal_output_dir / "formal_execution_started.json").exists():
        raise RuntimeError("v26 adaptation started before formal freeze")
    if (repo / "results/online/proximal_v26/final/final_test.json").exists():
        raise RuntimeError("v26 final outcome exists before formal freeze")
    ...
    calibration_checks = {
        ...
        == PRECALIBRATION_REVISION
        and pre.get("prospective_freeze", {}).get(
        ...
        "selected_height": calibration.get("selected_riser_height_m")
        == selected["riser_height_m"],
        "selected_clearance_slope": selected.get("clearance_barrier_slope")
        == CLEARANCE_BARRIER_SLOPE,
        ...
        "paired_row_count": len(paired_rows) == CALIBRATION_EPISODES,
    ...
    if not all(calibration_checks.values()):
        raise RuntimeError(f"formal v26 calibration mismatch: {calibration_checks}")
    ...
    payload = {
        ...
        "learning_semantics": {
            ...
            "teacher_action_already_in_actor_coordinates": True,
