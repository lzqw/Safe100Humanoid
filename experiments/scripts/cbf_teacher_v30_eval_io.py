"""Shared sequential evaluation and paired aggregation helpers for v30."""
...
def write_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    if not rows:
        raise ValueError("cannot write an empty v30 CSV")
    path.parent.mkdir(parents=True, exist_ok=True)
    temporary = path.with_name(f".{path.name}.tmp")
...
def evaluate_condition(
    *,
    ...
    if episodes < 1 or episodes % batch_size:
        raise ValueError("v30 episodes must divide exactly by evaluation batch")
    ...
    for repeat in range(episodes // batch_size):
        ...
        if summary is None:
            command = [
                sys.executable,
                str(repo / "experiments/scripts/evaluate_cbf_teacher_v30.py"),
                "--repo",
                str(repo),
                "--protocol",
                str(protocol),
            ...
            if completed.returncode:
                ...
                raise RuntimeError(
                    f"v30 evaluation failed for {context}/{condition}:\n{diagnostic}"
                )
            summary = json.loads(output_json.read_text())
        with output_csv.open(newline="") as handle:
            batch_rows = list(csv.DictReader(handle))
        if len(batch_rows) != batch_size:
            raise RuntimeError(f"incomplete v30 rows for {context}/{condition}")
        summaries.append(summary)
        rows.extend(batch_rows)
    rows.sort(key=identity)
    if len({identity(row) for row in rows}) != len(rows):
        raise RuntimeError(f"duplicate v30 identity in {context}/{condition}")
...
def aggregate_condition(
    *,
    ...
    if not summaries or not rows:
        raise ValueError("v30 aggregate inputs must be non-empty")
    ...
    if any(
        ...
        raise RuntimeError(f"v30 actor changed inside {context}/{condition}")
    return aggregate
...
def assert_paired(
    ...
    *,
    ...
    if any(value != identity_lists[0] for value in identity_lists[1:]):
        raise RuntimeError(f"v30 paired identities differ for {label}")
    ...
    if any(value != signatures[0] for value in signatures[1:]):
        raise RuntimeError(f"v30 initial state signatures differ for {label}")
...
def paired_ci(
    ...
    *,
    ...
    if len(baseline) != len(final) or not baseline:
        raise ValueError("v30 paired CI requires equal non-empty rows")
...
def paired_repairs_regressions(
    ...
    if len(baseline) != len(final):
        raise ValueError("v30 repair counts require paired rows")
