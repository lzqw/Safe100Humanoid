"""Run the frozen four-condition paired v26 final evaluation."""
...
from cbf_teacher_v26_protocol import (
    ...
    validate_v26_calibrated_context,
)
...
def _evaluate(
    *,
    ...
    for repeat in range(FINAL_REPEATS):
        ...
        if summary is None:
            ...
            if completed.returncode:
                ...
                raise RuntimeError(
                    f"v26 final condition {condition} failed:\n{diagnostic}"
                )
            summary = json.loads(output_json.read_text())
        with output_csv.open(newline="") as handle:
            batch_rows = list(csv.DictReader(handle))
        if len(batch_rows) != EVAL_BATCH_SIZE:
            raise RuntimeError(f"v26 condition {condition} has incomplete episode rows")
    ...
    if any(
        ...
        raise RuntimeError(f"v26 condition {condition} changed actors between batches")
    if any(
        ...
        raise RuntimeError(
            f"v26 condition {condition} changed checkpoint or actor interface"
        )
...
def main() -> None:
    ...
    if subprocess.run(
        ...
        raise RuntimeError("v26 final audit requires a clean committed worktree")
    for path in (
        base_checkpoint,
        final_checkpoint,
        training_path,
    ...
    if file_sha256(base_checkpoint) != BASE_CHECKPOINT_SHA256:
        raise RuntimeError("v26 final audit base checkpoint differs from pi0")
    context = validate_v26_calibrated_context(json.loads(context_path.read_text()))
    ...
    if (
        ...
        raise RuntimeError("v26 final audit inputs have the wrong protocol")
    if training.get("final_checkpoint_sha256") != file_sha256(final_checkpoint):
        raise RuntimeError("v26 final audit checkpoint differs from fixed round 8")
    if training.get("final_policy_rule") != "round 8 actor, never best-so-far":
        raise RuntimeError(
            "v26 final audit was not given the unconditional round-8 actor"
        )
    if training.get("protocol", {}).get("sha256") != file_sha256(protocol_path):
        raise RuntimeError("v26 final audit protocol differs from the training freeze")
    if protocol.get("implementation_boundary", {}).get("git_commit") != training.get(
        "protocol", {}
    ).get("implementation_commit"):
        raise RuntimeError(
            "v26 final audit implementation boundary differs from training"
        )
    ...
    if set(bound_sources) != set(SOURCE_FILES) or any(
        ...
        raise RuntimeError("v26 final evaluation source differs from formal freeze")
    output_dir.mkdir(parents=True, exist_ok=True)
    started = output_dir / "final_evaluation_started.json"
    if started.exists() and not args.resume:
        raise RuntimeError("v26 final evaluation already started; use --resume")
    ...
    height = float(context["shift"]["riser_height_m"])
    ...
    if any(value != signatures[0] for value in signatures[1:]):
        raise RuntimeError("v26 four final conditions do not share initial states")
    ...
    if any(value != identities[0] for value in identities[1:]):
        raise RuntimeError("v26 four-condition episode identities differ")
    if (
        len(identities[0]) != FINAL_EPISODES
        or len(set(identities[0])) != FINAL_EPISODES
    ):
        raise RuntimeError("v26 final episode identities are incomplete or duplicated")
