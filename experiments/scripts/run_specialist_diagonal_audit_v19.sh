#!/usr/bin/env bash
set -euo pipefail

ROOT="${SAFE100_HUMANOID_ROOT:-/home/carla/LZQW/SAFE100/humanoid}"
REPO="${SAFE100_MJLAB_REPO:-$ROOT/worktrees/v19_formal}"
PYTHON="${SAFE100_PYTHON:-$ROOT/workspace/conda_env/bin/python}"
BASELINE="${SAFE100_BASELINE_CHECKPOINT:-$ROOT/artifacts/retention_v13/arm_b_state_retention/accepted_final.pt}"
CONTEXT_DIR="${SAFE100_SPECIALIST_CONTEXT_DIR:-$ROOT/artifacts/specialist_v19/contexts}"
TRAINING_ROOT="${SAFE100_SPECIALIST_TRAINING_ROOT:-$ROOT/artifacts/specialist_v19/training}"
PROTOCOL="${SAFE100_V19_PROTOCOL_FILE:-$REPO/results/online/specialist_v19/protocol.json}"
PROTOCOL_COMMIT="${SAFE100_V19_PROTOCOL_COMMIT:?set SAFE100_V19_PROTOCOL_COMMIT to the frozen v19 commit}"
OUTPUT="${SAFE100_V19_AUDIT_OUTPUT_DIR:-$ROOT/artifacts/specialist_v19/diagonal_audit}"
LOG="${SAFE100_V19_AUDIT_LOG_PATH:-$ROOT/logs/specialist_v19/diagonal_audit.log}"

cd "$REPO"
test "$(git rev-parse HEAD)" = "$PROTOCOL_COMMIT"
git diff --quiet
git diff --cached --quiet
mkdir -p "$OUTPUT" "$(dirname "$LOG")"
export MUJOCO_GL=egl
export PYTHONUNBUFFERED=1

"$PYTHON" experiments/scripts/audit_specialists_diagonal_v19.py \
  --repo "$REPO" \
  --base-policy-checkpoint "$BASELINE" \
  --context-dir "$CONTEXT_DIR" \
  --training-root "$TRAINING_ROOT" \
  --protocol-file "$PROTOCOL" \
  --protocol-commit "$PROTOCOL_COMMIT" \
  --output-dir "$OUTPUT" \
  --adaptation-seeds 43 143 243 343 443 \
  --eval-batch-size 128 \
  --target-episodes 512 \
  --d0-episodes 256 \
  --bootstrap-samples 10000 \
  --audit-seed 5100000 \
  --bootstrap-seed 6000000 \
  --device cuda:0 \
  2>&1 | tee "$LOG"
