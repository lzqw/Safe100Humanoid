"""Independent fixed-eight-round v21 adaptation with the v19-R4 PPO core."""
...
from specialist_v21_tables import write_training_tables
...
def _parse_args() -> argparse.Namespace:
  ...
  parser.add_argument(
    "--method-role", choices=("control", "v21"), required=True
  )
  parser.add_argument(
    "--matched-success-preservation-beta", type=float, required=True
  )
  ...
  parser.add_argument(
    "--candidate-confirm-episodes",
    type=int,
    default=CONFIRMATION_EPISODES_PER_BLOCK,
  )
  parser.add_argument(
    "--candidate-confirm-blocks", type=int, default=CONFIRMATION_BLOCKS
  )
  parser.add_argument(
...
def _validate_protocol(args: argparse.Namespace) -> None:
  ...
  if mismatches:
    raise ValueError(f"v21 replay/candidate protocol mismatch: {mismatches}")
  if args.method_role == "control":
    if args.matched_success_preservation_beta != 0.0:
      raise ValueError("v21 control requires beta=0")
  elif args.matched_success_preservation_beta not in BETA_GRID:
    raise ValueError("v21 method beta must be a frozen-grid value")
  positive = (
    args.num_envs,
    args.rollout_steps,
    args.maximum_rounds,
  ...
  if min(positive) < 1:
    raise ValueError("v21 counts and capacities must be positive")
  if not args.smoke and (
    args.protocol_file is None or args.protocol_commit is None
  ):
    raise ValueError("formal/development v21 requires its frozen protocol")
  if not args.smoke:
    required = {
    ...
    if size_mismatches or float_mismatches or args.fall_redistribution_horizon != 100:
      raise ValueError(
        "formal v21 configuration mismatch: "
...
def _validate_algorithm(runner, *, beta: float) -> dict[str, Any]:
  alg = runner.alg
  new_feature_groups = [
    group
  ...
  checks = {
    ...
    "matched_success_preservation_beta": (
      alg.matched_success_preservation_beta
    ),
  ...
  valid = (
    ...
    and checks["actor_layer_multipliers"] == list(V21_ACTOR_LAYER_MULTIPLIERS)
    ...
    and math.isclose(checks["matched_success_preservation_beta"], beta)
...
def _validate_frozen_protocol(
  *,
  ...
  if current_commit != args.protocol_commit:
    raise RuntimeError("v21 training HEAD differs from the frozen protocol commit")
  if not tracked_clean:
    raise RuntimeError("formal/development v21 requires a clean tracked worktree")
  try:
    relative = protocol_path.relative_to(repo)
  except ValueError as exc:
    raise RuntimeError("v21 training protocol must be inside the repository") from exc
  ...
  if protocol_sha256 != _file_sha256(protocol_path):
    raise RuntimeError("v21 training protocol differs from its committed blob")
  sealed = protocol.get("sealed_inputs", {})
  declared_context = sealed.get("contexts", {}).get(args.context_id, {})
  expected_revision = 1 if args.development else 2
  expected_status = (
    "prospectively_frozen_before_development_beta_selection"
    if args.development
    else "prospectively_frozen_before_formal_adaptation"
  )
  checks = {
    "protocol_id": protocol.get("protocol_id") == PROTOCOL_ID,
    "protocol_revision": protocol.get("protocol_revision") == expected_revision,
    ...
    "declared_runtime_seed": protocol.get("adaptation_seeds", {}).get(
      args.context_id
    )
    == args.seed,
    "development_context_role": (
      context.get("formal_context") is (not args.development)
    ),
    ...
    "method_role": (
      args.matched_success_preservation_beta == 0.0
      if args.method_role == "control"
      else (
        not args.development
        or args.matched_success_preservation_beta > 0.0
      )
    ),
    "beta_grid": protocol.get("development", {}).get("beta_grid")
    == list(BETA_GRID),
    "beta_allowed": (
      args.matched_success_preservation_beta in BETA_GRID
      if args.development
      else args.matched_success_preservation_beta
      in (0.0, protocol.get("formal", {}).get("selected_beta"))
    ),
  }
  failed = [name for name, passed in checks.items() if not passed]
  if failed:
    raise RuntimeError(f"v21 frozen training protocol check failed: {failed}")
  return {
...
def main() -> None:
  ...
  context = (
    ...
    if args.smoke
    else load_calibrated_v21_context(context_path)
  )
  if context["context_id"] != args.context_id:
    raise ValueError("v21 frozen context ID differs from the requested deployment")
  if context["specialist_mode"] != args.mode:
    raise ValueError("v21 frozen context mode differs from requested specialist")
  ...
  alg_cfg.matched_success_preservation_beta = (
    args.matched_success_preservation_beta
  )
  ...
  structural_checks = _validate_algorithm(
    runner, beta=args.matched_success_preservation_beta
  )
  ...
  _save_checkpoint(
    ...
    metadata={
      ...
      "monitor_set_accessed_during_training": False,
    },
  ...
  for round_index in range(1, args.maximum_rounds + 1):
    ...
    if screened_best is not None:
      ...
      success_deltas = []
      fall_deltas = []
      for block_index in range(args.candidate_confirm_blocks):
        block_seed = confirmation_seed + 1_000 * block_index
        ...
        if old_confirm["initial_state_signatures"] != candidate_confirm[
          ...
          raise RuntimeError("v21 confirmation block is not strictly paired")
        success_delta = float(
        ...
        fall_deltas.append(fall_delta)
        confirmation_blocks.append(
          {
            "block": block_index + 1,
            "seed": block_seed,
            "episodes": args.candidate_confirm_episodes,
            "old": old_confirm,
      ...
      target_gate_accepted, confirm_reasons, confirm_deltas = (
        confirmation_block_gate(
          success_deltas,
          fall_deltas,
          mean_kl=float(screened_best["update_metrics"]["mean_kl"]),
          finite=_finite_actor_state(screened_best["state"]),
        )
      )
      confirmation = {
        "seed_base": confirmation_seed,
        "independent_block_count": args.candidate_confirm_blocks,
        "episodes_per_block": args.candidate_confirm_episodes,
  ...
  result = {
    ...
    "learning_core": (
      "v19-R4 collection with v21 Local Matched-Success Preservation"
      if args.matched_success_preservation_beta > 0.0
      else "v19-R4 matched-success PPO-advantage control"
    ),
    ...
    "actor_objective": (
      "PPO on normal and failure-precursor transitions plus beta times "
      "KL(pi_round_reference || pi_candidate) on matched-success transitions"
      if args.matched_success_preservation_beta > 0.0
      else "v20-style PPO on normal/failure/matched-success transitions"
    ),
    "critic_objective": "value loss on every transition",
    "broad_d0_or_global_retention_bank_used": False,
    "advantage_normalization": (
      "separate normal/failure groups for the v21 actor; matched successes "
      "have no actor advantage"
      if args.matched_success_preservation_beta > 0.0
      else "separate normal/failure/success groups across both batches"
    ),
    ...
    "candidate_selection": {
      ...
      "independent_confirmation_blocks": args.candidate_confirm_blocks,
      "paired_episodes_per_confirmation_block": (
        args.candidate_confirm_episodes
      ),
      "independent_confirmation_paired_episodes": (
        args.candidate_confirm_blocks * args.candidate_confirm_episodes
      ),
      "candidate_episode_total": (
        3 * args.candidate_screen_episodes
        + args.candidate_confirm_blocks * args.candidate_confirm_episodes
      ),
      "confirmation_gate": {
        "mean_success_delta_strictly_above": 0.0,
        "minimum_positive_success_blocks": 2,
        "maximum_mean_fall_increase": 0.03,
        "maximum_kl_strictly_below": 0.01,
    ...
    "learning_curve_protocol": {
      "saved_checkpoints": [
        f"post_round_{round_index:03d}.pt"
        for round_index in range(args.maximum_rounds + 1)
      ],
      "monitor_set_accessed_during_training": False,
      "monitor_evaluation_runs_only_after_training": True,
    },
