"""Evaluate one v30 policy/filter/context condition on fixed initial episodes."""
...
def _initial_state_signature(obs, base_env, action_term, command_term) -> str:
    ...
    if terrain is None:
        raise RuntimeError("v30 initial signature requires stair terrain")
    tensors = (
        obs["actor"],
...
def main() -> None:
    ...
    if args.num_envs != args.num_episodes or args.num_envs < 1:
        raise ValueError("v30 evaluation uses one initial episode per environment")
    repo = args.repo.resolve()
    protocol_path = args.protocol.resolve()
    checkpoint = args.checkpoint.resolve()
    if not checkpoint.is_file() or not protocol_path.is_file():
        raise FileNotFoundError("v30 evaluation input is missing")
    if subprocess.run(
        ...
        raise RuntimeError("v30 evaluation requires a clean committed worktree")
    protocol = json.loads(protocol_path.read_text())
    if protocol.get("protocol_id") != PROTOCOL_ID:
        raise RuntimeError("v30 evaluation protocol id differs")
    ...
    if context_spec != expected_spec:
        raise RuntimeError("v30 evaluation context differs from protocol")
    ...
    if runner_cls is None:
        raise RuntimeError("v30 task has no standard inference runner")
    ...
    try:
        ...
        if int(runner.alg.actor.obs_dim) != 405:
            raise RuntimeError("v30 evaluator actor is not the original 405-D policy")
        ...
        if not isinstance(action_term, HigherRiserCbfAction):
            raise TypeError("v30 evaluator did not build the fixed CBF action")
        ...
        if bool(active.any()) or len(completed) != args.num_episodes:
            raise RuntimeError("v30 evaluator did not finish every initial episode")
    ...
    if summary["teacher_reprojection_max_abs_error"] > 1.0e-6:
        raise RuntimeError("v30 evaluator detected teacher transform corruption")
    if summary["swing_selection_mismatch_count"] != 0:
        raise RuntimeError("v30 evaluator detected swing-foot routing corruption")
