...
def main() -> None:
  ...
  parser.add_argument("--correction-distillation-weight", type=float, default=0.1)
  ...
  agent_cfg.algorithm.pre_intervention_weight = (
    0.0 if args.task_first_constrained else agent_cfg.algorithm.pre_intervention_weight
  )
  agent_cfg.algorithm.intervention_advantage_weight = (
    0.0
    if args.task_first_constrained
    else agent_cfg.algorithm.intervention_advantage_weight
  )
  ...
  obs, _ = env.reset()
  if args.task_first_constrained:
    # Mechanical smoke budgets only; formal runs derive these from paired
    # baseline evaluation in online_refine_stairs.py.
    runner.alg.set_cost_budgets(
      fall_rate=0.10,
      intervention_per_riser=1.0,
      intervention_slack=1.05,
    )
  ...
  with torch.no_grad():
    ...
    if args.task_first_constrained:
      multiplier = runner.alg.update_cost_multipliers()
    else:
      multiplier = {}
    advantage = runner.alg.prepare_constrained_advantages()
