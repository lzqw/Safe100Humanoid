...
OUTPUT="${SAFE100_OUTPUT_DIR:-$ROOT/artifacts/failure_focused_v15/train_seed${SEED}}"
LOG="${SAFE100_LOG_PATH:-$ROOT/logs/failure_focused_v15/train_seed${SEED}.log}"

case "$SEED" in
  42|142|242) ;;
  *) echo "formal v15 seed must be 42, 142, or 242" >&2; exit 2 ;;
esac
...
"$PYTHON" experiments/scripts/brief_ppo_refine_stairs.py \
  ...
  --failure-discovery-max-rollouts 4 \
