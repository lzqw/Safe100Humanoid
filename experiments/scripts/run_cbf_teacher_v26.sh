...
REPO=${REPO:?set REPO to the committed v26 checkout}
...
PRECALIBRATION_PROTOCOL=${PRECALIBRATION_PROTOCOL:-$RESULT_ROOT/precalibration_protocol.json}
EXTERNAL_ROOT=${EXTERNAL_ROOT:?set EXTERNAL_ROOT outside the Git repository}

case "${1:-}" in
  calibrate)
    shift
    "$PYTHON" "$REPO/experiments/scripts/calibrate_cbf_teacher_v26.py" \
      --repo "$REPO" \
  ...
  train)
    shift
    "$PYTHON" "$REPO/experiments/scripts/refine_cbf_teacher_v26.py" \
      --repo "$REPO" \
      --base-checkpoint "$BASE_CHECKPOINT" \
  ...
  audit)
    shift
    "$PYTHON" "$REPO/experiments/scripts/audit_cbf_teacher_v26.py" \
      --repo "$REPO" \
      --base-checkpoint "$BASE_CHECKPOINT" \
