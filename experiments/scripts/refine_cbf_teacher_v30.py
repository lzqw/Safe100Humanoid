"""Run one fixed v30 smoke, development arm, or formal adaptation run."""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--phase", choices=("smoke", "development", "formal"), required=True
    )
    parser.add_argument("--arm", choices=tuple(ARMS), required=True)
    parser.add_argument("--context", choices=("DEV", *FORMAL_CONTEXTS), required=True)
...
def _validate_protocol(
    ...
    *,
    ...
    expected_status = (
        "frozen_before_v30_development"
        if phase in ("smoke", "development")
        else "frozen_before_v30_formal"
    )
    ...
    checks = {
        ...
        == 0,
        "arm_matches": protocol.get("development_arms", {}).get(arm)
        == arm_parameters(arm),
        "context_matches": protocol.get("contexts", {}).get(context)
        == environment_parameters(context),
    }
    if phase == "smoke":
        checks.update({"smoke_arm": arm == "A5", "smoke_context": context == "DEV"})
    elif phase == "development":
        checks["development_context"] = context == "DEV"
    else:
        selected = protocol.get("formal", {}).get("selected_teacher", {}).get("arm")
        checks.update(
            {
                "formal_context": context in FORMAL_CONTEXTS,
                "formal_arm": arm in ("A0", selected),
                "formal_seed_frozen": protocol.get("formal", {})
                .get("adaptation_seeds", {})
                .get(context)
                == adaptation_seed("formal", context),
            }
        )
    if not all(checks.values()):
        raise RuntimeError(f"v30 protocol validation failed: {checks}")
    return protocol, {
        "path": str(path),
...
def _configure_algorithm(agent_cfg, arm: str, *, smoke: bool) -> None:
    ...
    agent_cfg.algorithm = CbfTeacherV30PpoAlgorithmCfg(
        ...
        num_learning_epochs=1 if smoke else ACTOR_EPOCHS,
        ...
        # The one-off functional smoke switches this to one immediately after
        ...
        v30_smoke_all_arm_diagnostics=smoke,
    )


def _structural_audit(
    runner, env_cfg, shift: dict[str, Any], *, smoke: bool
) -> dict[str, Any]:
    ...
    expected = {
        ...
        "runtime_cbf": True,
        "plant_identity": True,
        "actor_epochs": 1 if smoke else ACTOR_EPOCHS,
        "critic_epochs": 1 if smoke else CRITIC_EPOCHS,
    ...
    if mismatches:
        raise RuntimeError(f"v30 structural audit failed: {mismatches}")
    return values
...
def _save_checkpoint(
    ...
    payload["infos"] = {"cbf_teacher_v30": metadata}
...
def _smoke_checks(summary: dict[str, Any]) -> dict[str, bool]:
    metrics = summary["rounds"][0]["metrics"]
    losses = metrics.get("smoke_all_arm_teacher_losses", {})
    target_shape = metrics.get("teacher_target_shape", [])
    shapes = metrics.get("teacher_tensor_shapes", {})
    action_shape = [SMOKE_STEPS, SMOKE_ENVS, 12]
    scalar_shape = [SMOKE_STEPS, SMOKE_ENVS]
    checks = {
        "all_six_arm_configurations_instantiate": len(
            metrics.get("smoke_all_arm_configurations_instantiated", [])
        )
        == 6
        and set(losses) == set(ARMS),
        ...
        and shapes.get("done") == [SMOKE_STEPS, SMOKE_ENVS, 1],
        "full_nll_and_residual_huber_finite": all(
...
def main() -> None:
    ...
    if _git(repo, "status", "--porcelain"):
        raise RuntimeError("v30 execution requires a clean committed worktree")
    if not checkpoint.is_file() or not protocol_path.is_file():
        raise FileNotFoundError("v30 base checkpoint or protocol is missing")
    ...
    if smoke:
        ...
        seed = adaptation_seed(args.phase, args.context)
        rounds, num_envs, rollout_steps = ROUNDS, NUM_ENVS, ROLLOUT_STEPS
    marker = output_dir / "execution_started.json"
    if marker.exists() or output_dir.exists():
        raise RuntimeError(f"v30 run output already exists: {output_dir}")
    ...
    from src.tasks.stairs_cbf.environment_v30 import configure_v30_context
    from src.tasks.stairs_cbf.teacher_v30 import CbfTeacherV30Runner

    env_cfg = load_env_cfg(TASK_ID, play=True)
    shift = configure_v30_context(
        env_cfg,
        context=args.context,
        runtime_filter=True,
    ...
    _configure_algorithm(agent_cfg, args.arm, smoke=smoke)
    ...
    try:
        structural = _structural_audit(runner, env_cfg, shift, smoke=smoke)
