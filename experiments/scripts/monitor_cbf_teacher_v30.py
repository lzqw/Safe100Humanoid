"""Read-only F1 round-0-through-8 monitor after all formal v30 work."""
...
def main() -> None:
    ...
    if _git(repo, "status", "--porcelain"):
        raise RuntimeError("v30 monitor requires a clean worktree")
    protocol = json.loads(protocol_path.read_text())
    formal_audit = json.loads(formal_audit_path.read_text())
    if (
        protocol.get("status") != "frozen_before_v30_formal"
        or formal_audit.get("protocol_id") != PROTOCOL_ID
        or not formal_audit.get("complete")
    ):
        raise RuntimeError("v30 monitor can run only after the formal audit")
    selected = protocol["formal"]["selected_teacher"]["arm"]
    if selected not in TEACHER_ARMS:
        raise RuntimeError("v30 monitor selected arm differs")
    if (output / "F1_checkpoint_curve.csv").exists() and not args.resume:
        raise RuntimeError("v30 monitor output already exists")
    output.mkdir(parents=True, exist_ok=True)
    method_arms = {"control": "A0", "teacher": selected}
    ...
    if checkpoint_hashes_after != checkpoint_hashes_before:
        raise RuntimeError("v30 read-only monitor changed a checkpoint")
