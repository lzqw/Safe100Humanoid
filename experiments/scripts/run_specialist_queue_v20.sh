...
printf '%s mode=%s event=queue_started commit=%s\n' \
...
for seed in 73 173 273 373 473; do
  ...
  SAFE100_MJLAB_REPO="$REPO" \
    bash experiments/scripts/run_specialist_v20.sh \
    2>&1 | tee -a "$QUEUE_LOG"
...
SAFE100_V20_PROTOCOL_COMMIT="$PROTOCOL_COMMIT" \
SAFE100_MJLAB_REPO="$REPO" \
  bash experiments/scripts/run_specialist_diagonal_audit_v20.sh \
  2>&1 | tee -a "$QUEUE_LOG"
