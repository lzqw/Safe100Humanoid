"""Evaluate one v25 policy/filter condition on paired initial episodes."""
...
def main() -> None:
    ...
    if args.num_envs != args.num_episodes or args.num_envs < 1:
        raise ValueError("v25 requires one initial episode per environment")
    ...
    if "deployable_failure" in env_cfg.observations:
        raise RuntimeError("v25 evaluator contains a forbidden actor observation")
    if "specialist_failure_signal" in env_cfg.rewards:
        raise RuntimeError("v25 evaluator contains a forbidden specialist reward")
    ...
    if runner_cls is None:
        raise RuntimeError("v25 task has no runner")
    ...
    try:
        ...
        if int(runner.alg.actor.obs_dim) != 405:
            raise RuntimeError("v25 evaluator actor is not the original 405-D policy")
        actor_hash = actor_state_sha256(runner.alg.actor.state_dict())
        policy = runner.get_inference_policy(args.device)
        obs, _ = env.reset()
        action_term = base_env.action_manager.get_term("joint_pos")
        if not isinstance(action_term, SwingUnderResponseCbfAction):
            raise TypeError("v25 evaluator did not build its additive action term")
        ...
        with torch.inference_mode():
            for _ in range(maximum_steps):
                ...
                overlap = extras["v25_toe_riser_overlap"].bool()
                ...
                reprojection = extras["v25_teacher_reprojection_error"]
                selection_match = extras["v25_swing_selection_matches"].bool()
        ...
        if bool(active.any()) or len(completed) != args.num_episodes:
            raise RuntimeError("v25 evaluator did not finish every initial episode")
        ...
        summary = {
            ...
            "swing_underresponse_gain": args.gain,
    ...
    if summary["teacher_reprojection_max_abs_error"] > 1.0e-6:
        raise RuntimeError("v25 evaluator detected teacher reprojection corruption")
    if summary["swing_selection_mismatch_count"] != 0:
        raise RuntimeError("v25 evaluator detected divergent swing-foot selection")
